...
void setInitailConditions()
{
	...
	double massOfCeres = 9.383e20; // kg
	double diameterOfCeres = 940.0; // km
	double densityOfCeres = massOfCeres / ((PI / 6.0) * diameterOfCeres * diameterOfCeres * diameterOfCeres); // kg/km^3
	...
	MassUnitConverter = massOfCeres/NUMBER_OF_BODIES;
	...
	for(int i = 0; i < NUMBER_OF_BODIES; i++)
	{
		BodyMass[i] = ((float)rand() / (float)RAND_MAX) * MassUnitConverter; // Random mass
		BodyRadius[i] = pow((3.0 * BodyMass[i]) / (4.0 * PI * densityOfCeres), 1.0 / 3.0); // Calculate radius based on mass
...
void drawPicture()
{
	...
	for(int i = 0; i < NUMBER_OF_BODIES; i++)
	{
		...
		glPushMatrix();
			...
			glutSolidSphere(BodyRadius[i]/2.0, 30, 30);
		glPopMatrix();
	}
...
void getForces()
{
	...
	float sphereRadius = BodyRadius[i];
	...
	{	
		...
		for(int j = 0; j < i; j++)
		{
			...
			if(d.w < BodyRadius[i])
			{
				...
				//if(inOut < 0.0) magnitude = kSphere*(BodyRadius[i] - d.w); // If inOut is negative the sphere are converging.
				//else magnitude = kSphereReduction*kSphere*(BodyRadius[i] - d.w); // If inOut is positive the sphere are diverging.
				...
				// was at impact.
				magnitude = GavityConstant*BodyMass[i]*BodyMass[i]/(BodyRadius[i]*BodyRadius[i]);
				Force[i].x += magnitude*unit.x;
				Force[i].y += magnitude*unit.y;
				Force[i].z += magnitude*unit.z;
			...
			else
			{
				...
				magnitude = GavityConstant*BodyMass[i]*BodyMass[i]/(d.w*d.w);
				Force[i].x += magnitude*unit.x;
				Force[i].y += magnitude*unit.y;
				Force[i].z += magnitude*unit.z;
...
int main(int argc, char** argv)
{
	...
	// Clip plains
	Near = 0.2;
	Far = 50.0*BodyRadius[i];

	//Where your eye is located
	EyeX = 0.0;
	EyeY = 0.0;
	EyeZ = 25.0*BodyRadius[i];
