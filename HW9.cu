...
void KeyPressed(unsigned char key, int x, int y)
{
	...
	// Make a key that will propel the asteriod into your wall
	  if (key == 'f')
        ...
        drawPicture();
        printf("\n Fired the balls towards the wall!\n");
    }
...
void drawPicture()
{
	...
	glBegin(GL_QUADS); //this makes it solid
	...
	{	
		...
		for(int j = 0; j < i; j++)
		{
			...
			if(d < SphereDiameter)
			{
				...
				if(inOut < 0.0) magnitude = sphereReplusionScalar*kSphere*(SphereDiameter - d); // If inOut is negative the sphere are converging.
				else magnitude = sphereReplusionScalar*kSphereReduction*kSphere*(SphereDiameter - d); // If inOut is positive the sphere are diverging.
